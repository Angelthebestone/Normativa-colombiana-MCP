...
- **Reutilizar lo que ya está instalado antes de escribir nada.** `pedir` de `src/http.ts` para todo HTTP y `cargar` de `src/parse.ts` para HTML: traen ritmo por dominio, reintentos, decodificación y cadena de certificados. Un `fetch` directo se salta las cuatro cosas. Comprobar la documentación y los tipos de una dependencia antes de dar por hecho que le falta algo.
