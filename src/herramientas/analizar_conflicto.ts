...
export async function evidenciaDe(cita: string, sobre?: string): Promise<Evidencia> {
  ...
  try {
    ...
    if (reformas.length) {
      base.reformas = reformas.map((x) => `- MODIFICADO por ${x.norma} nota literal: «${x.literal}»`)
    }
...
export function formatear(evA: Evidencia, evB: Evidencia, sobre?: string): string {
  ...
  for (const [etiqueta, ev] of [
    ...
    if (ev.noEncontrada || !ev.parseada) continue
  ...
  bloques.push('', 'Conflicto POTENCIAL, no conclusión jurídica: esto es evidencia reunida, no un análisis de contradicciones. Verifica en los enlaces antes de actuar.')
  return bloques.join('\n')
}
